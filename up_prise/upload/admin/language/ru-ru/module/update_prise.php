<?php

// Text
$_['text_module']      = 'Modules';
$_['text_total']      = 'Order Totals';
$_['text_payment']      = 'Payments';
$_['text_shipping']      = 'Shipping';
$_['text_feed']        = 'Feeds';
$_['text_success']     = 'Success: You have modified module Y!';

$_['download']                              = 'Загрузка';
// Error
$_['error_permission']                      = 'Warning: You do not have permission to modify module Y!';
// Heading
$_['heading_title']                         = 'Загрузка прайса';

// edit_prise start
$_['tab_manufacture']                       = 'Наценка по производителям';
$_['tab_category']                          = 'Наценка по категория';

$_['text_etap0']                            = 'Для загрузки прайса на сайт и обновления цен выполните следующие шаги:';
$_['text_etap1m']                           = '1. Выберите подготовленный файл';
$_['text_etap1c']                           = '1. Выберите категорию из списка';
$_['text_etap2']                            = '2. Примените установленные наценки по производителям или по категориям.<br> Вам просто необходимо нажать на соотвествующую кнопку.';
$_['text_etap3']                            = '3. Примените наценку товаров';
$_['text_procent']                          = 'Введите процент накрутки';
$_['text_cheslo']                           = 'Введите добавочное число';
$_['text_no_data']                          = 'Нет данных';

$_['button_prim']                           = 'Применить';
$_['button_del']                            = 'Отменить';
$_['button_red']                            = 'Редактировать';
$_['button_m']                              = 'Наценка по производителям';
$_['button_c']                              = 'Наценка по категориям';
$_['button_back']                           = 'Назад';

// Table
$_['table_header']                          = 'Таблица наценок которые уже применены на сайте';
$_['table_id']                              = 'id';
$_['table_name']                            = 'Наименование';
$_['table_procent']                         = 'Процент наценки';
$_['table_cheslo']                          = 'Добавочное число';
$_['table_del']                             = 'Удалить наценку';
$_['table_up']                              = 'Редактировать наценку';
$_['table_prin']                            = 'Применить изменения';

$_['del_help']                              = 'Удалить без подтверждения';



?>
